...
export function ChatSidebar() {
  ...
  return (
    <aside
      className={clsx(
        "relative flex h-full min-h-0 shrink-0 flex-col border-l-2 border-[#2a3a28] bg-[#0c1410] transition-[width] duration-200 ease-out",
        open ? "w-[300px]" : "w-12",
      )}
    >
      <button
      ...
      {open ? (
        <>
          <p className="shrink-0 border-b border-amber-400/20 bg-amber-400/10 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wide text-amber-200">
            Rain {formatRemain(nextRainAt - now)} · {CHAT_RAIN_WINNERS}×{CHAT_RAIN_PRIZE} SH
          </p>
