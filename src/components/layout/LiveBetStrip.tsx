...
export function LiveBetStrip() {
  ...
  const rows = useMemo(() => {
    ...
    if (tab === "lucky") return list.filter((p) => multiplier(p) > LUCKY_X).slice(0, 12);
    if (tab === "high") return list.filter((p) => p.wagered >= HIGH_ROLLER_WL).slice(0, 12);
    if (tab === "mine") return list.filter((p) => isLocalPlayerName(p.name)).slice(0, 12);
  ...
  return (
    <section className="shrink-0 border-t border-white/[0.06] bg-[#0a1212]">
      <div className="flex items-center gap-1 overflow-x-auto px-3 pt-2 scrollbar-thin">
        ...
        <span className="ml-auto hidden text-[10px] text-slate-600 sm:inline">Live · ~3s</span>
      </div>
      <div className="max-h-[9.5rem] overflow-y-auto px-2 py-1.5 scrollbar-thin">
        {rows.length === 0 ? (
              ...
              {rows.map((p) => {
                ...
                return (
                  <tr key={p.id} className="border-t border-white/[0.04] text-slate-300">
                    <td className="whitespace-nowrap px-2 py-1 font-mono text-slate-500">{timeAgo(p.at, now)}</td>
                    <td className="max-w-[7rem] truncate px-2 py-1 font-semibold text-slate-200">{name}</td>
                    <td className="hidden px-2 py-1 text-slate-400 sm:table-cell">{ACTIVITY_GAME_LABELS[p.game]}</td>
                    <td className="px-2 py-1">
                      <CashAmount wl={p.wagered} iconClassName="h-3 w-3" />
                    </td>
                    <td className={clsx("px-2 py-1", p.won > p.wagered ? "text-emerald-300" : "text-slate-400")}>
                      <CashAmount wl={p.won} iconClassName="h-3 w-3" />
                    </td>
                    <td className={clsx("px-2 py-1 font-mono", multi > LUCKY_X ? "text-amber-300" : "text-slate-500")}>
