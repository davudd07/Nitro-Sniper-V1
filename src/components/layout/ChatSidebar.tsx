...
export function ChatSidebar() {
  ...
  return (
    <aside
      className={clsx(
        "relative flex shrink-0 flex-col border-l-2 border-[#2a3a28] bg-[#0c1410] transition-[width] duration-200 ease-out",
        open ? "w-[300px]" : "w-12",
      )}
    >
      <button
        ...
        className="absolute -left-3 top-4 z-20 grid h-6 w-6 place-items-center rounded-md border-2 border-[#3d5a3a] bg-[#152018] text-emerald-200 shadow-[2px_2px_0_#050805]"
        title={open ? "Collapse chat" : "Open chat"}
      >
        {open ? <ChevronRight className="h-3.5 w-3.5" /> : <ChevronLeft className="h-3.5 w-3.5" />}
      </button>

      <div className={clsx("flex items-center gap-2 border-b border-white/8 px-3 py-4", !open && "justify-center px-1")}>
      ...
      {open ? (
        <>
          <div className="flex-1 space-y-2.5 overflow-y-auto px-3 py-3 scrollbar-thin">
            {messages.map((m) => (
              <div key={m.id} className={clsx("rounded-md border border-white/8 bg-black/25 px-2.5 py-1.5", m.you && "border-fuchsia-400/30")}>
          ...
          <form
            className="flex gap-1.5 border-t border-white/8 p-2"
            onSubmit={(e) => {
              e.preventDefault();
              submit();
            }}
